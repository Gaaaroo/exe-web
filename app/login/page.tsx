...
export default function LoginPage() {
  ...
  return (
    <div
      className={`min-h-screen bg-background flex items-center justify-center px-4 sm:px-6 relative overflow-hidden ${cormorant.variable}`}
    >
      ...
      <div
        ...
        style={{
          backgroundImage:
            'radial-gradient(circle at 1px 1px, hsl(var(--gold) / 0.65) 1px, transparent 0)',
      ...
      <motion.div
        ...
        className='relative w-full max-w-[400px] rounded-4xl p-px bg-linear-to-br from-gold/40 via-white/15 to-transparent shadow-[0_0_40px_-8px_hsl(var(--gold)/0.4),0_0_60px_-16px_hsl(var(--primary)/0.12),0_25px_50px_-12px_rgba(0,0,0,0.5)]'
      >
        <div className='rounded-4xl bg-card/90 backdrop-blur-xl p-8 sm:p-10 border border-gold/15'>
          <h1
            className={`text-center text-3xl sm:text-4xl font-semibold mb-2 bg-clip-text text-transparent ${cormorant.className}`}
            style={{
              backgroundImage:
                'linear-gradient(180deg, hsl(var(--gold-glow)) 0%, hsl(var(--parchment)) 100%)',
            }}
          >
            Đăng nhập / Đăng ký
          </h1>
          ...
          {error === 'auth' && (
            <div className='rounded-4xl border border-destructive/40 bg-destructive/10 py-3 px-4 mb-6'>
              <p className='font-body text-sm text-parchment text-center'>
          ...
          {error === 'state' && (
            <div className='rounded-4xl border border-gold/30 bg-gold/10 py-3 px-4 mb-6'>
              <p className='font-body text-xs text-foreground text-center leading-relaxed'>
                Phiên đăng nhập hết hạn hoặc thiếu cấu hình. Vào Supabase
                Dashboard → Authentication → URL Configuration, thêm{' '}
                <strong>Redirect URLs</strong>:{' '}
                <code className='text-[10px] break-all block mt-2 text-gold/80'>
